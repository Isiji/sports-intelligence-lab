...
from app.odds.selection_mapper import SELECTION_ALIASES
...
def run_historical_value_backtest(
    ...
    if use_only_matches_with_odds:
        ...
        df = df[
            df["match_id"].isin(odds_match_ids)
        ].copy()
    ...
    while start_test_index + test_window_size <= len(df):
        ...
        print(
            ...
            f"test={len(test_df)} "
            f"start_index={start_test_index}"
        )
        ...
        windows.append(
            {
                ...
                "wins": window_wins,
                "hit_rate": round(
                    window_wins / window_bets,
                    4,
                ) if window_bets else 0.0,
