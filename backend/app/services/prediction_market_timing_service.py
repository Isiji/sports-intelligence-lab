...
def _parse_kickoff(value: Any) -> datetime | None:
    ...
    if isinstance(value, datetime):
        ...
        kickoff = datetime(value.year, value.month, value.day)
...
def analyze_prediction_timing(
    *,
    ...
    kickoff_eat_dt = _parse_kickoff(kickoff_value)
    ...
    minutes = int((kickoff_eat_dt - current_time.astimezone(EAT)).total_seconds() // 60)
    ...
    if minutes < 0:
        ...
        action = "AVOID_OR_RECHECK_ODDS"
        reason = "Too close to kickoff; odds may be unstable."
        ...
        reason = "Match is still far away; monitor odds movement."
